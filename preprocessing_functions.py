...
def replace_weird_values(X):
    ...
    partial_no_replace = {}
    partial_no_replace.update(
        {63: {77, 88, 99, 777, 888, 999}, 263: {77, 88, 99, 777, 888, 999}}
    )
...
def features_with_vocabs_above_threshold(feature_mask, categorical_mask, train_vocabs):
    ...
    treshold = 2
    large_vocab_info = [
        (int(orig_cat_indices[i]), len(v))
        for i, v in enumerate(train_vocabs)
        if len(v) > treshold
    ]
    large_vocab_indices = [idx for idx, _ in large_vocab_info]
    if large_vocab_indices:
        print(
            f"preprocess_data: categorical features with vocab size>{treshold} (original indices -> vocab_size): {large_vocab_info}",
            flush=True,
        )
    else:
        print(
            "preprocess_data: no categorical feature has vocab size > {treshold}",
            flush=True,
        )
...
def impute_train_test(
    x_train_cont, x_test_cont, x_train_cat, x_test_cat, cast_categorical_to_int=True
):
    """
