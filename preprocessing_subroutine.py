...
def preprocess_data(verbose=True):
    ...
    if os.path.exists(final_cache_path):
        ...
        if verbose:
            print(
                f"preprocess_data: loaded final cached data from {final_cache_path}",
                flush=True,
            )
    ...
    if os.path.exists(cache_path):
        ...
        if verbose:
            print(
                f"preprocess_data: loaded cached raw data from {cache_path}", flush=True
            )
    else:
        ...
        if verbose:
            print(
                f"preprocess_data: loaded data from CSV and saved raw cache to {cache_path}",
                flush=True,
            )
    ...
    if verbose:
        print(
            f"preprocess_data: replaced weird values (train shape {x_train_replaced.shape}, test shape {x_test_replaced.shape})",
            flush=True,
        )
    ...
    if verbose:
        print(
            f"preprocess_data: suppressed features -> kept {kept_after_suppress}/{P_orig} features",
            flush=True,
        )

    # Step 3: Feature filtering (remove features with too many NaNs)
    x_train_filtered, feature_mask = nan_feature_filter(
        x_train_suppressed, cutoff_ratio=0.2, prev_mask=suppressed_mask
    )
    # feature_mask is a boolean mask relative to the original feature indices
    x_test_filtered = x_test_replaced[
        :, feature_mask
    ]  # Apply same mask to test set (indexing over original columns)
    kept_after_nan_filter = feature_mask.sum()
    if verbose:
        print(
            f"preprocess_data: nan feature filter -> kept {kept_after_nan_filter}/{P_orig} original features",
            flush=True,
        )
    ...
    if verbose:
        print(
            f"preprocess_data: split cont/cat -> cont {continuous_mask.sum()}, cat {categorical_mask.sum()} (relative to filtered features)",
            flush=True,
        )
    ...
    if verbose:
        print(
            f"preprocess_data: one-hot encoded categorical -> train enc shape {x_train_cat_enc.shape}, test enc shape {x_test_cat_enc.shape}",
            flush=True,
        )
    ...
    (
        ...
        means,
        modes,
    ) = impute_train_test(x_train_cont, x_test_cont, x_train_cat_enc, x_test_cat_enc)
    if verbose:
        print(
            f"preprocess_data: imputation done. cont shapes: train {x_train_cont_imp.shape}, test {x_test_cont_imp.shape}; cat shapes: train {x_train_cat_imp.shape}, test {x_test_cat_imp.shape}",
            flush=True,
        )
    ...
    if verbose:
        print(
            f"preprocess_data: recombined final arrays -> train {x_train_final.shape}, test {x_test_final.shape}",
            flush=True,
        )
    if verbose:
        print(
            f"preprocess_data: finished (total NaNs in train final: {np.isnan(x_train_final).sum()}, test final: {np.isnan(x_test_final).sum()})",
            flush=True,
        )
    ...
    try:
        ...
        if verbose:
            print(
                f"preprocess_data: saved final cached data to {final_cache_path}",
                flush=True,
            )
        ...
        if verbose:
            print(
                f"preprocess_data: warning saving final cache failed: {e}", flush=True
            )
